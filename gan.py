...
epochs = 15
save_path = data_set_name + '_ds_' + str(epochs) + 'e'
...
def show_images(noise, epoch=None):
    ...
    if epoch != None:
        plt.savefig(f'{save_path}/epoch-{epoch}.png')
...
frames[0].save(save_path + '/' + str(data_set_name) + '_training.gif', format='GIF', append_images=frames[1:], save_all=True, duration=500, loop=0)

discriminator.save(save_path + '/' + str(data_set_name) + '_fcdiscriminator.hdf5')
generator.save(save_path + '/' + str(data_set_name) + '_fcgenerator.hdf5')